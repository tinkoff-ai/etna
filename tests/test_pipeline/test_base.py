...
def test_validate_predict_timestamps_calculate_values(
    ts_name, expected_start_timestamp, expected_end_timestamp, request
):
    ts = request.getfixturevalue(ts_name)

    start_timestamp, end_timestamp = BasePipeline._validate_predict_timestamps(ts=ts)

    assert start_timestamp == expected_start_timestamp
    assert end_timestamp == expected_end_timestamp


def test_validate_predict_timestamps_fail_early_start(example_tsds):
    ...
    with pytest.raises(ValueError, match="Value of start_timestamp is less than beginning of some segments"):
        _ = BasePipeline._validate_predict_timestamps(ts=example_tsds, start_timestamp=start_timestamp)


def test_validate_predict_timestamps_fail_late_end(example_tsds):
    ...
    with pytest.raises(ValueError, match="Value of end_timestamp is more than ending of dataset"):
        _ = BasePipeline._validate_predict_timestamps(ts=example_tsds, end_timestamp=end_timestamp)


def test_validate_predict_timestamps_fail_start_later_than_end(example_tsds):
    ...
    with pytest.raises(ValueError, match="Value of end_timestamp is less than start_timestamp"):
        _ = BasePipeline._validate_predict_timestamps(
