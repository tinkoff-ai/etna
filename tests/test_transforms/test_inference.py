...
@pytest.fixture
def regular_ts(example_tsds) -> TSDataset:
    return example_tsds
...
@pytest.fixture
def ts_to_fill(example_tsds) -> TSDataset:
    df = example_tsds.to_pandas()
    df.loc[["2020-01-01", "2020-01-03", "2020-01-08", "2020-01-09"], pd.IndexSlice[:, "target"]] = np.NaN
...
class TestTransformFutureSubsetSegments:
    ...
    def _test_transform_future_subset_segments(self, ts, transform, segments):
        # select subset of tsdataset
        train_ts, future_ts = ts.train_test_split(test_size=14)
        segments = list(set(segments))
...
class TestTransformTrainNewSegments:
    ...
    def _test_transform_train_new_segments(self, ts, transform, train_segments, expected_changes):
        ...
        columns_before_transform = set(flat_test_df.columns)
        columns_after_transform = set(flat_transformed_test_df.columns)
        ...
        for column in columns_to_check_changes:
            if not flat_transformed_test_df[column].equals(flat_test_df[column]):
    ...
    def test_transform_train_new_segments_failed_not_implemeted_per_segment(self, transform, dataset_name, request):
        ts = request.getfixturevalue(dataset_name)
        self._test_transform_train_new_segments(ts, transform, train_segments=["segment_2"], expected_changes={})
...
class TestTransformFutureNewSegments:
    ...
    def _test_transform_future_new_segments(self, ts, transform, train_segments, expected_changes):
        ...
        train_ts = select_segments_subset(ts=deepcopy(history_ts), segments=train_segments)
        test_ts = select_segments_subset(ts=deepcopy(future_ts), segments=forecast_segments)
        train_df = train_ts.to_pandas()
        test_df = test_ts.to_pandas()
    ...
    def test_transform_future_new_segments_failed_not_implemeted_per_segment(self, transform, dataset_name, request):
        ts = request.getfixturevalue(dataset_name)
        self._test_transform_future_new_segments(ts, transform, train_segments=["segment_2"], expected_changes={})
