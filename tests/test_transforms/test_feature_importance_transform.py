...
def test_mrmr_right_len(relevance_method, clustering_method, top_k, ts_with_regressors):
    ...
    mrmr = MRMRFeatureSelectionTransform(
        relevance_method=relevance_method, top_k=top_k, clustering_method=clustering_method, n_clusters=2
    )
...
def test_mrmr_fails_negative_parameters():
    ...
    with pytest.raises(ValueError, match="positive integer"):
        MRMRFeatureSelectionTransform(relevance_method=StatisticsRelevanceTable(), top_k=-1)
    with pytest.raises(ValueError, match="greater than"):
        MRMRFeatureSelectionTransform(relevance_method=StatisticsRelevanceTable(), top_k=1, n_clusters=1)
...
def test_mrmr_fails(ts_with_regressors):
    ...
    mrmr = MRMRFeatureSelectionTransform(relevance_method=StatisticsRelevanceTable(), top_k=4, freq="D", n_clusters=25)
