...
def simple_df() -> pd.DataFrame:
    ...
    df_1["exog"] = [6.0, 7.0, 8.0, 9.0, 10.0,11.,12.]
    df_2["segment"] = "Omsk"
    df_2["target"] = [10.0, 20.0, 30.0, 40.0, 50.0, np.NAN, np.NAN]
    df_2["exog"] = [60.0, 70.0, 80.0, 90.0, 100.0,110.,120.]
    classic_df = pd.concat([df_1, df_2], ignore_index=True)
...
def test_mean_segment_encoder_transform(example_tsds):
    horizon = 5
    model = LinearMultiSegmentModel()
    lag = LagTransform(in_column="target", lags=[5])
    encoder = MeanSegmentEncoderTransform()
    encoder2 = SegmentEncoderTransform()

    train, test = example_tsds.train_test_split(test_size=horizon)
    train.fit_transform([lag])
    model.fit(train)
    future = train.make_future(horizon)
    pred = model.forecast(future)

    train, test = example_tsds.train_test_split(test_size=horizon)
    train.fit_transform([encoder2])
    model.fit(train)
    future = train.make_future(horizon)
    pred_segment_encoding = model.forecast(future)
    ...
    assert metric(pred_segment_encoding,test) > metric(pred_mean_segment_encoding,test)
