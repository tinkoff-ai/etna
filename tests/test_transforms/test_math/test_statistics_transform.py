...
def test_interface_simple(simple_ts_for_agg: pd.DataFrame, class_name: Any, out_column: str):
    transform = class_name(window=3, out_column=out_column, in_column="target")
    res = transform.fit_transform(ts=simple_ts_for_agg)
    result_column = out_column if out_column is not None else transform.__repr__()
    assert sorted(res.to_pandas()["segment_1"]) == sorted([result_column] + ["target"])


@pytest.mark.parametrize("out_column", (None, "test_q"))
def test_interface_quantile(simple_ts_for_agg: pd.DataFrame, out_column: str):
