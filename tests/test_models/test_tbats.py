...
def sinusoid_ts():
    horizon = 2
    sinusoid_ts_1 = pd.DataFrame({'segment' : np.zeros(8), 'timestamp': pd.date_range(start='1/1/2018', end='1/08/2018'), 'target': [np.sin(i) for i in range(8)]})
    sinusoid_ts_2 = pd.DataFrame({'segment' : np.ones(8), 'timestamp': pd.date_range(start='1/1/2018', end='1/08/2018'), 'target': [np.sin(i + 1) for i in range(8)]})
...
@pytest.mark.parametrize(
     "model", (TBATSPerSegmentModel(), BATSPerSegmentModel())
)
