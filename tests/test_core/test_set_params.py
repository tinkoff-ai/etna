...
def test_base_mixin_set_params_changes_params_estimator():
    ...
    expected_dict = {"iterations":1000, "depth":8, "learning_rate":1e-3, 'logging_level':'Silent', 'kwargs':{},
...
def test_base_mixin_set_params_changes_params_pipeline():
    ...
    expected_dict = {'_target_':'etna.pipeline.pipeline.Pipeline', 'horizon':5,
    'model': {'_target_':'etna.models.catboost.CatBoostMultiSegmentModel', 'depth':8, 'iterations':1000,
    'kwargs':{}, 'learning_rate':0.001, 'logging_level':'Silent'},
    'transforms':{'_target_':'etna.transforms.math.add_constant.AddConstTransform',
    'in_column':'column', 'inplace':True, 'value':1}}
...
def test_base_mixin_set_params_with_nonexistent_attributes_estimator():
    ...
    expected_dict = {"iterations":1000, "depth":10, 'logging_level':'Silent', 'kwargs':{},
     '_target_':'etna.models.catboost.CatBoostMultiSegmentModel'}
    obtained_dict = catboost_model.to_dict()
    assert obtained_dict == expected_dict
...
def test_base_mixin_set_params_with_nonexistent_attributes_pipeline():
    pipeline = Pipeline(model=CatBoostMultiSegmentModel(iterations=1000, depth=10), transforms=(), horizon=5)
    pipeline.set_params(**{"incorrect_estimator":"value", "model.incorrect_attribute":"value",
    "model.incorrect_nesting.depth":"value"})
    expected_dict = {'_target_':'etna.pipeline.pipeline.Pipeline', 'horizon':5,
    'model': {'_target_':'etna.models.catboost.CatBoostMultiSegmentModel', 'depth':10, 'iterations':1000,
    'kwargs':{}, 'logging_level':'Silent'}, "transforms":()}
    obtained_dict = pipeline.to_dict()
    assert obtained_dict == expected_dict
